...
- Install numpy (for the grayscale death effect):
  ```powershell
...
airdrop_survival/
  assets/
    sounds/
      ...
      bgm.mp3|bgm.wav   # background music (prefer mp3, fall back to wav)
  src/
  ...
  audio.py    # reserved for future audio helpers
...
- Background BGM: if `assets/sounds/bgm.mp3` exists, it will be used; otherwise the game looks for `bgm.wav`. No auto-generated urgent tracks are used anymore.
...
airdrop_survival/
  ...
  success.wav            # 成功结局音乐
  failure.wav            # 失败结局音乐
  bgm.mp3|bgm.wav        # 背景音乐（优先 mp3，若无则尝试 wav）
...
- 背景 BGM：若存在 `assets/sounds/bgm.mp3` 则优先使用；否则使用 `bgm.wav`。目前不再自动合成紧张 BGM，占位音轨已移除。
